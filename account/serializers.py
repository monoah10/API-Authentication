...
class UserRegistrationSerializer(serializers.ModelSerializer):
    #this password2 is for confrimations purpose
    ...
    #validating password and confriming password
    def validate(self, attrs):
        password=attrs.get('password')
        password2=attrs.get('password2')
        if password != password2:
            raise serializers.ValidationError("Password doesn't match please check the password and try again")
        return attrs
...
class SendPasswordRestEmailSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if User.objects.filter(email=email).exists():
            ...
            # send email
            body="Click the link to rest your password" + link
            data={
